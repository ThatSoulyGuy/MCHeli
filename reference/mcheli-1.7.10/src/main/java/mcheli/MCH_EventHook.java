...
public class MCH_EventHook extends W_EventHook {
   ...
   public void entitySpawn(EntityJoinWorldEvent event) {
      if (W_Lib.isEntityLivingBase(event.entity) && !W_EntityPlayer.isPlayer(event.entity)) {
         event.entity.renderDistanceWeight = event.entity.renderDistanceWeight * MCH_Config.MobRenderDistanceWeight.prmDouble;
         ...
         if (b) {
            MCH_Lib.Log(event.entity, "### EntityJoinWorldEvent Error:Player invalid rotation pitch(" + e.rotationPitch + ")");
         ...
         if (ac.getAcInfo() != null) {
            if (!ac.isDestroyed()) {
               ...
               if (attackEntity == null) {
                  ...
                  event.ammount = event.ammount * ac.getAcInfo().damageFactor;
               } else if (W_Entity.isEqual(attackEntity, event.entity)) {
                  ac.attackEntityFrom(event.source, event.ammount * 2.0F);
                  event.ammount = event.ammount * ac.getAcInfo().damageFactor;
               } else if (ac.isMountedEntity(attackEntity)) {
   ...
   public MCH_EntityAircraft getRiddenAircraft(Entity entity) {
      ...
      if (ac == null) {
         List list = entity.worldObj.getEntitiesWithinAABB(MCH_EntityAircraft.class, entity.boundingBox.expand(50.0, 50.0, 50.0));
         if (list != null) {
