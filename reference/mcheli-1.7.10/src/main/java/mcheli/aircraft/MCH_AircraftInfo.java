...
public abstract class MCH_AircraftInfo extends MCH_BaseInfo {
   ...
   public boolean isValidData() throws Exception {
      ...
      if (this.isUAV) {
         ...
         if (this.seatList.size() == 0) {
            MCH_SeatInfo s = new MCH_SeatInfo(Vec3.createVectorHelper(0.0, 0.0, 0.0), false);
            this.seatList.add(s);
         }
      }
      ...
      if (item.compareTo("displayname") == 0) {
            ...
            this.entityRackList
               .add(
                  new MCH_SeatRackInfo(
                     ...
                     this.toDouble(s[2]),
                     this.toDouble(s[3]),
                     new MCH_AircraftInfo.CameraPosition(this.toVec3(s[4], s[5], s[6]).addVector(0.0, 1.5, 0.0)),
                     range,
                     para,
                     yaw,
                     pitch,
         ...
         if (s.length >= 4) {
            ...
            for (int i = 2; i < s.length; i++) {
               this.wheels.add(new MCH_AircraftInfo.Wheel(Vec3.createVectorHelper(x, y, this.toFloat(s[i]))));
            }
         ...
         if (!item.equalsIgnoreCase("AddPartSlideRotLG") && s.length >= 6) {
            ...
            if (s.length >= 8) {
               ...
               n.rot2 = Vec3.createVectorHelper(this.toFloat(s[7]), this.toFloat(s[8]), this.toFloat(s[9]));
            }
         ...
         if (item.equalsIgnoreCase("AddPartSlideRotLG") && s.length >= 9) {
            ...
            n.slide = Vec3.createVectorHelper(this.toFloat(s[0]), this.toFloat(s[1]), this.toFloat(s[2]));
            this.landingGear.add(n);
         }
   ...
   public class CameraPosition {
      ...
      public CameraPosition() {
         this(Vec3.createVectorHelper(0.0, 0.0, 0.0));
      }
   }
   ...
   public class Flare {
      ...
      public Vec3 pos = Vec3.createVectorHelper(0.0, 0.0, 0.0);
   }
