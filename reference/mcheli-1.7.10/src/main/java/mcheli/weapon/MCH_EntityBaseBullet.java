...
public abstract class MCH_EntityBaseBullet extends W_Entity {
   ...
   public void setBomblet() {
      this.isBomblet = 1;
      this.sprinkleTime = 0;
      this.dataWatcher.updateObject(31, (byte)1);
   }
   ...
   public void updateSplash() {
      if (this.getInfo() != null) {
         if (this.getInfo().power > 0) {
            if (!W_WorldFunc.isBlockWater(this.worldObj, (int)(this.prevPosX + 0.5), (int)(this.prevPosY + 0.5), (int)(this.prevPosZ + 0.5))
               && W_WorldFunc.isBlockWater(
                  this.worldObj, (int)(this.posX + 0.5), (int)(this.posY + 0.5), (int)(this.posZ + 0.5)
               )) {
   ...
   protected void onUpdateCollided() {
      ...
      if (this.getInfo().delayFuse > 0) {
         ...
         Entity entity = null;
         List list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.addCoord(mx, my, mz).expand(21.0, 21.0, 21.0));
         double d0 = 0.0;
      ...
      if (!this.worldObj.isRemote) {
         ...
         for (int i = 0; i < p / 3.0F; i++) {
            MCH_ParticlesUtil.spawnParticleTileCrack(
               ...
               m.blockY,
               m.blockZ,
               m.hitVec.xCoord + (this.rand.nextFloat() - 0.5) * p / 10.0,
               m.hitVec.yCoord + 0.1,
               m.hitVec.zCoord + (this.rand.nextFloat() - 0.5) * p / 10.0,
               -this.motionX * p / 2.0,
               p / 2.0F,
               -this.motionZ * p / 2.0
            );
   ...
   public void onImpactEntity(Entity entity, float damageFactor) {
      if (!entity.isDead) {
         MCH_Lib.DbgLog(this.worldObj, "MCH_EntityBaseBullet.onImpactEntity:Damage=%d:" + entity.getClass(), this.getPower());
         ...
         if (this instanceof MCH_EntityBullet
            ...
            && this.shootingEntity.ridingEntity instanceof MCH_EntitySeat) {
   ...
   public void newFAExplosion(double x, double y, double z, float exp, float expBlock) {
      MCH_Explosion.ExplosionResult result = MCH_Explosion.newExplosion(
         this.worldObj, this, this.shootingEntity, x, y, z, exp, expBlock, true, true, this.getInfo().flaming, false, 15
      );
   ...
   public void newExplosion(double x, double y, double z, float exp, float expBlock, boolean inWater) {
      ...
      if (!inWater) {
         result = MCH_Explosion.newExplosion(
            ...
            exp,
            expBlock,
            this.isBomblet == 1 ? this.rand.nextInt(3) == 0 : true,
            true,
   ...
   public boolean attackEntityFrom(DamageSource ds, float par2) {
      if (this.isEntityInvulnerable()) {
         return false;
      } else if (!this.worldObj.isRemote && par2 > 0.0F && ds.getDamageType().equalsIgnoreCase("thrown")) {
         ...
         MovingObjectPosition m = new MovingObjectPosition(
            ...
            (int)(this.posZ + 0.5),
            0,
            Vec3.createVectorHelper(this.posX + 0.5, this.posY + 0.5, this.posZ + 0.5)
         );
