...
public class MCH_ConfigGui extends W_GuiContainer {
         ...
         switch (button.id) {
            ...
            case 401:
               ...
               for (int i = 0; i < list.size(); i++) {
                  if (list.get(i) instanceof MCH_EntityAircraft) {
                     MCH_EntityAircraft var9 = (MCH_EntityAircraft)list.get(i);
                     if (var9.getAcInfo() != null) {
                        var9.getAcInfo().reload();
                        var9.changeType(var9.getAcInfo().name);
                        var9.onAcInfoReloaded();
                     }
                  }
               }
            ...
            case 400:
               ...
               if (ac != null && ac.getAcInfo() != null) {
                  ...
                  list = this.mc.theWorld.loadedEntityList;
   ...
   public void drawTexturedModalRectRotate(
      ...
      tessellator.addVertexWithUV(-width / 2.0, height / 2.0, this.zLevel, uLeft * fw, (vTop + vHeight) * fh);
      tessellator.addVertexWithUV(width / 2.0, height / 2.0, this.zLevel, (uLeft + uWidth) * fw, (vTop + vHeight) * fh);
      tessellator.addVertexWithUV(width / 2.0, -height / 2.0, this.zLevel, (uLeft + uWidth) * fw, vTop * fh);
      tessellator.addVertexWithUV(-width / 2.0, -height / 2.0, this.zLevel, uLeft * fw, vTop * fh);
      tessellator.draw();
      GL11.glPopMatrix();
   }
}
