...
public final class MchLockTracker {
    ...
    // TEMPORARY lock-on diagnostics (#25 client path). Flip DEBUG=false / delete once the S-75 lock issue is resolved.
    private static final org.slf4j.Logger LOG = com.mojang.logging.LogUtils.getLogger();
    private static final boolean DEBUG = true;
    private static int dbgTick;

    /** Log every potential target in range with the per-gate result, so a live client shows exactly why the lock
     *  fails (classify = the air/ground/kind filter; cone = aiming; los = line-of-sight; range). Throttled. */
    private static void debugAcquire(Player user, AbstractMchVehicle vehicle, MCH_WeaponInfo wi,
                                     boolean air, boolean ground, boolean water) {
        Level level = user.level();
        double lr = wi.lockRange;
        AABB box = user.getBoundingBox().inflate(lr, lr, lr);
        int potential = 0;
        for (Entity e : level.getEntitiesOfClass(Entity.class, box, x -> x != user && x != vehicle)) {
            if (!(e instanceof LivingEntity) && !(e instanceof AbstractMchVehicle)) {
                continue;
            }
            potential++;
            double dx = e.getX() - user.getX(), dy = e.getY() - user.getY(), dz = e.getZ() - user.getZ();
            double d = Math.sqrt(dx * dx + dy * dy + dz * dz);
            boolean ong = isEntityOnGround(e.level(), e);
            boolean classify = canLockEntity(e, user, vehicle, wi, air, ground, water);
            boolean cone = MCH_LockGeometry.inLockCone(user.getX(), user.getY(), user.getZ(),
                user.getYRot(), user.getXRot(), e.getX(), e.getY() + e.getBbHeight() / 2.0, e.getZ(), wi.lockAngle);
            boolean los = hasLineOfSight(level, user, e);
            LOG.info("[LOCK-DBG] cand={} dist={} onGround={} classify={} inRange={} cone={} los={} | userYaw={} userPitch={} lockAngle={}",
                e.getType().builtInRegistryHolder().key().location(), String.format("%.1f", d), ong, classify,
                d < lr, cone, los, String.format("%.1f", user.getYRot()), String.format("%.1f", user.getXRot()), wi.lockAngle);
        }
        LOG.info("[LOCK-DBG] wi={} lockRange={} potentialTargetsInRange={} lockCount={}", wi.type, lr, potential, lockCount);
    }
    ...
    public static void tick(AbstractMchVehicle vehicle, int seat, int weaponIndex, MCH_WeaponInfo wi, boolean fireHeld) {
        ...
        if (DEBUG && (dbgTick++ % 10) == 0) {
            debugAcquire(user, vehicle, wi, canLockInAir, canLockOnGround, canLockInWater);
        }
