...
public class MCH_Config {
   ...
   public MCH_Config(String minecraftPath, String cfgFile) {
      ...
      ItemRecipe_UavStation = new MCH_ConfigPrm[]{
         new MCH_ConfigPrm("ItemRecipe_UavStation", "\"III\", \"IDI\", \"IRI\", I, iron_ingot, D, diamond, R, redstone_block"),
         new MCH_ConfigPrm("ItemRecipe_UavStation2", "\"IDI\", \"IRI\", I, iron_ingot, D, diamond, R, redstone")
      };
   ...
   public void correctionParameter() {
      ...
      for (MCH_ConfigPrm p : CommandPermission) {
         MCH_Config.CommandPermission cpm = new MCH_Config.CommandPermission(p.prmString);
         if (!cpm.name.isEmpty()) {
      ...
      for (MCH_Config.DamageFactor df : DamageFactorList) {
         if (df.list.size() <= 0) {
            ...
            for (MCH_Config.DamageEntity n : df.list) {
               if (n.name.isEmpty()) {
                  foundCommon = true;
                  break;
               }
   ...
   public MCH_Config.DamageEntity newDamageEntity(String s) {
      ...
      if (splt.length == 1) {
         return new MCH_Config.DamageEntity(Double.parseDouble(splt[0]), "");
      } else {
         return splt.length == 2 ? new MCH_Config.DamageEntity(Double.parseDouble(splt[0]), splt[1]) : new MCH_Config.DamageEntity(1.0, "");
      }
   }
