...
public class MCH_DraftingTableGui extends W_GuiContainer {
   ...
   public MCH_DraftingTableGui(EntityPlayer player, int posX, int posY, int posZ) {
      ...
      MCH_Lib.DbgLog(player.worldObj, "MCH_DraftingTableGui.MCH_DraftingTableGui");
   }
   ...
   public void initGui() {
      ...
      for (int ix = 0; ix < this.screenButtonList.size(); ix++) {
         list = this.screenButtonList.get(ix);
   ...
   public void onGuiClosed() {
      super.onGuiClosed();
      MCH_Lib.DbgLog(this.thePlayer.worldObj, "MCH_DraftingTableGui.onGuiClosed");
   }
   ...
   protected void drawGuiContainerForegroundLayer(int mx, int my) {
      ...
      if (this.getScreenId() == 0) {
         ...
         if (list.size() > 0) {
            this.drawHoveringText(list, mx - 30, my - 0, this.fontRendererObj);
         }
      }
         ...
         for (int r = 0; r < 3; r++) {
            for (int c = 0; c < 2; c++) {
               if (index + i < this.getCurrentList().getRecipeListSize()) {
                  int rx = 110 + 140 * c;
                  int ry = 20 + 70 * r;
                  String s = this.getCurrentList().getRecipe(index + i).getRecipeOutput().getDisplayName();
            ...
            if (recipe.getRecipeOutput().getItem() != null) {
               if (recipe instanceof ShapedRecipes) {
                  ...
                  for (int i = 0; i < rcp.recipeItems.size(); i++) {
                     this.drawItemStack((ItemStack)rcp.recipeItems.get(i), x + i % 3 * 18, y + i / 3 * 18);
                  }
               }
