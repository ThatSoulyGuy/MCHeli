      ...
      if (!this.isDead && !this.worldObj.isRemote && this.getCountOnUpdate() > 1 && this.getInfo() != null && this.explosionPower > 0) {
         ...
         if (pDist > 0.1) {
            ...
            for (int i = 0; i < list.size(); i++) {
               ...
               if (this.canBeCollidedEntity(entity1) && entity1.getDistanceSqToEntity(this) < pDist * pDist) {
                  MCH_Lib.DbgLog(this.worldObj, "MCH_EntityBullet.onUpdate:proximityFuse:" + entity1);
                  this.posX = (entity1.posX + this.posX) / 2.0;
                  this.posY = (entity1.posY + this.posY) / 2.0;
                  this.posZ = (entity1.posZ + this.posZ) / 2.0;
                  MovingObjectPosition mop = W_MovingObjectPosition.newMOP(
