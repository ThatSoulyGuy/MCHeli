...
public class MCH_ItemWrench extends W_Item {
      ...
      if (!player.worldObj.isRemote && count < this.getMaxItemUseDuration(stack) && count % 20 == 0) {
         ...
         if (ac != null && ac.getHP() > 0 && ac.repair(10)) {
            ...
            W_WorldFunc.MOD_playSoundEffect(
               player.worldObj, (int)ac.posX, (int)ac.posY, (int)ac.posZ, "wrench", 1.0F, 0.9F + rand.nextFloat() * 0.2F
            );
         }
      }
   }
   ...
   private MovingObjectPosition getMouseOver(EntityLivingBase user, float tick) {
      ...
      Vec3 vec33 = null;
      float f1 = 1.0F;
      List list = user.worldObj
         .getEntitiesWithinAABBExcludingEntity(
            user, user.boundingBox.addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand(f1, f1, f1)
         );
      double d2 = d1;
