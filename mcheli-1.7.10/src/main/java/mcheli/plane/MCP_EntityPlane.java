...
public class MCP_EntityPlane extends MCH_EntityAircraft {
         ...
         if (!(this.getHP() >= this.getMaxHP() * 0.5)) {
            if (this.getPlaneInfo() != null) {
               ...
               for (int ri = 0; ri < rotorNum; ri++) {
                  ...
                  Vec3 rotor_pos = this.getPlaneInfo().rotorList.get(ri).pos;
   ...
   public void onUpdate_Particle2SpawnSmoke(int ri, double x, double y, double z, float size, boolean spawnSmoke) {
      ...
      for (int i = 0; i < num; i++) {
         ...
         MCH_ParticleParam prm = new MCH_ParticleParam(
            this.worldObj,
            "smoke",
            prev.xCoord + (x - prev.xCoord) * i / 3.0,
            prev.yCoord + (y - prev.yCoord) * i / 3.0,
            prev.zCoord + (z - prev.zCoord) * i / 3.0
         );
   ...
   public void onUpdate_ParticleLandingGear() {
      ...
      if (d > 0.01) {
         int x = MathHelper.floor_double(this.posX + 0.5);
         int y = MathHelper.floor_double(this.posY - 0.5);
         int z = MathHelper.floor_double(this.posZ + 0.5);
   ...
   public void onUpdate_ParticleNozzle() {
      if (this.planeInfo != null && this.planeInfo.haveNozzle()) {
         if (!(this.getCurrentThrottle() <= 0.1F)) {
            ...
            float pitch = this.getRotPitch();
            float roll = this.getRotRoll();
            Vec3 nozzleRot = MCH_Lib.RotVec3(0.0, 0.0, 1.0, -yaw - 180.0F, pitch - this.getNozzleRotation(), roll);
            ...
            for (MCH_AircraftInfo.DrawnPart nozzle : this.planeInfo.nozzles) {
               if (!(this.rand.nextFloat() > this.getCurrentThrottle() * 1.5)) {
                  ...
                  float a = 0.7F;
                  if (W_WorldFunc.getBlockId(
                        this.worldObj,
                        (int)(x + nozzleRot.xCoord * 3.0),
                        (int)(y + nozzleRot.yCoord * 3.0),
                        (int)(z + nozzleRot.zCoord * 3.0)
                     )
                     != 0) {
                     a = 2.0F;
                  }
                  ...
                  MCH_ParticleParam prm = new MCH_ParticleParam(
                     ...
                     y,
                     z,
                     nozzleRot.xCoord + (this.rand.nextFloat() - 0.5F) * a,
                     nozzleRot.yCoord,
                     nozzleRot.zCoord + (this.rand.nextFloat() - 0.5F) * a,
   ...
   private void onUpdate_Server() {
      ...
      if (!levelOff) {
         if (this.getNozzleRotation() <= 0.01F) {
            this.motionY = this.motionY + v.yCoord * throttle / 2.0;
         } else {
