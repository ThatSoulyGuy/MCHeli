...
public class MCH_EntityHeli extends MCH_EntityAircraft {
   ...
   protected void entityInit() {
      super.entityInit();
      this.dataWatcher.addObject(30, (byte)2);
   }
   ...
   public void setFoldBladeStat(byte b) {
      if (!this.worldObj.isRemote && b >= 0 && b <= 3) {
         this.dataWatcher.updateObject(30, b);
      }
   }
   ...
   protected void onUpdate_Control() {
      ...
      this.rotationRotor = this.rotationRotor + (1.0F - rp * rp * rp) * this.getAcInfo().rotorSpeed;
            ...
            if (this.getHeliInfo() != null) {
               ...
               if (rotorNum > 0) {
                  ...
                  for (int ri = 0; ri < rotorNum; ri++) {
                     Vec3 rotor_pos = this.getHeliInfo().rotorList.get(ri).pos;
                     float yaw = this.getRotYaw();
                     ...
                     for (double i = 0.0; i < num; i++) {
                        ...
                        if (p < this.rand.nextFloat() / 2.0F) {
                           ...
                           MCH_ParticleParam prm = new MCH_ParticleParam(
                              this.worldObj,
                              "smoke",
                              prev.xCoord + (x - prev.xCoord) * (i / num),
                              prev.yCoord + (y - prev.yCoord) * (i / num),
                              prev.zCoord + (z - prev.zCoord) * (i / num)
                           );
      ...
      if (!this.isHovering()) {
         ...
         if (dp == 0.0) {
            ...
            this.motionX = this.motionX
               + 0.1 * MathHelper.sin(yaw) * this.currentSpeed * -(pitch * pitch * pitch / 30000.0F) * this.getCurrentThrottle();
            this.motionZ = this.motionZ
               + 0.1 * MathHelper.cos(yaw) * this.currentSpeed * (pitch * pitch * pitch / 30000.0F) * this.getCurrentThrottle();
            double y = MathHelper.abs(this.getRotPitch()) + MathHelper.abs(this.getRotRoll());
            y *= 0.6F;
            if (y <= 50.0) {
               y = 1.0 - y / 50.0;
            } else {
